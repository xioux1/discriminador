...
- Para código PL/SQL: declarás siempre el modo de cada parámetro (IN, OUT o IN OUT). Nunca dejés un parámetro sin modo explícito.
- En funciones PL/SQL que señalan errores: no mezcles códigos numéricos de error con valores de datos reales retornados por la misma vía (RETURN). Usá un parámetro OUT para el resultado real y RAISE_APPLICATION_ERROR para los errores, o bien definí un rango de códigos de error que no pueda solaparse con ningún valor legítimo del dominio.
- Los códigos de error numéricos en PL/SQL deben ser contiguos y sin saltos; si la secuencia original es 1, 2, 3, la variante no puede saltar de 1 a 3 sin introducir el 2.
- PRAGMA EXCEPTION_INIT solo tiene efecto cuando la excepción se lanza mediante RAISE_APPLICATION_ERROR o corresponde a un error Oracle (ORA-XXXXX). No uses PRAGMA EXCEPTION_INIT en excepciones de usuario lanzadas con RAISE explícito; en ese contexto el pragma no tiene ningún efecto y confunde al lector.
