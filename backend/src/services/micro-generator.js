...
ESTRATEGIAS:
- FUNDAMENTO: El estudiante no sabe qué es el concepto, o su respuesta no tiene relación con él → identificá el término técnico MÁS ELEMENTAL que el estudiante desconoce dentro del concepto y preguntá qué es, nombrándolo directamente. Si el concepto es compuesto (ej: "ajuste de perplejidad en LSTM"), descomponelo al término más básico ignorado (ej: "LSTM" o "perplejidad").
- CONSECUENCIA: Mencionó algo relacionado pero le faltó la idea clave → preguntá qué pasaría sin ese concepto
- DISTINCIÓN: Confundió el concepto con otro similar → preguntá la diferencia puntual
- APLICACIÓN: Entiende en abstracto pero no en práctica → planteá un caso concreto técnico mínimo
- CORRECCIÓN: Dijo algo específicamente incorrecto → preguntá si su lógica resiste el caso más simple

IMPORTANTE:
- Usá FUNDAMENTO cuando el estudiante claramente no sabe qué es el concepto (respuesta en blanco, "no sé", off-topic, o menciona no conocer el término).
- NUNCA uses ANALOGÍA para conceptos técnicos, científicos, matemáticos o computacionales — no figura en las opciones.

Respondé SOLO en este formato (sin nada más):
DIAGNÓSTICO: <qué sabe y qué no sabe el estudiante, 1 oración>
ESTRATEGIA: <FUNDAMENTO|CONSECUENCIA|DISTINCIÓN|APLICACIÓN|CORRECCIÓN>
RAZÓN: <por qué esta estrategia es la mejor aquí, 1 oración>
TÉRMINO_ELEMENTAL: <si ESTRATEGIA=FUNDAMENTO: el término técnico más básico y atómico que el estudiante desconoce (ej: "LSTM", "perplejidad", "gradiente"); si otra estrategia: vacío>`,
    messages: [{
      role: 'user',
...
Concepto no entendido: "${concept}"`,
  ...
  const elementalTermMatch   = text.match(/TÉRMINO_ELEMENTAL:\s*(.+)/i);

  return {
    diagnosis:     diagnosisMatch?.[1]?.trim() ?? '',
    strategy:      strategyMatch?.[1]?.trim().toUpperCase() ?? 'FUNDAMENTO',
    reason:        reasonMatch?.[1]?.trim() ?? '',
    elementalTerm: elementalTermMatch?.[1]?.trim() ?? '',
  };
}

const STRATEGY_GUIDE = `CÓMO EJECUTAR CADA ESTRATEGIA:
- FUNDAMENTO: Identificá el término técnico MÁS ELEMENTAL ignorado e ignorá el contexto complejo del ejercicio original. Nombrá ese término directamente en la pregunta: "¿Qué es [término]?" o "¿Para qué se usa [término]?". PROHIBIDO construir escenarios. PROHIBIDO asumir que el estudiante ya sabe qué es.
- CONSECUENCIA: Preguntá qué pasaría si ese concepto no estuviera o no se aplicara.
- DISTINCIÓN: Preguntá en qué se diferencia del concepto con el que lo confundió.
- APLICACIÓN: Planteá una situación concreta y técnica mínima. Pedile qué haría en ese caso.
- CORRECCIÓN: Tomá lo que dijo y preguntá si su lógica funciona en el caso más simple posible.`;
...
export async function generateMicroCard({ prompt_text, expected_answer_text, subject, concept, user_answer = '', slotIndex = 0 }) {
  ...
  if (slotIndex === 0) {
    ...
    const elementalLine = (plan.strategy === 'FUNDAMENTO' && plan.elementalTerm)
      ? `Término elemental a preguntar: "${plan.elementalTerm}"\n`
      : '';
...
- No construyas escenarios complejos — usá el más básico posible.
- Si la estrategia es FUNDAMENTO: nombrá el término técnico elemental directamente en la pregunta ("¿Qué es X?" o "¿Para qué se usa X?"). PROHIBIDO asumir que el estudiante ya sabe qué es ese término. PROHIBIDO construir escenarios hipotéticos. PROHIBIDO usar el contexto complejo del ejercicio original.
- Para DISTINCIÓN y CORRECCIÓN: evitá nombrar el concepto si hacerlo revela la respuesta.${angleSection}
